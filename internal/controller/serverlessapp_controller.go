...
// It never imports any cloud SDK directly — all cloud calls go through CloudProvider.
...
func (r *ServerlessAppReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	logger.Info("reconciling", "app", req.NamespacedName)
...
func (r *ServerlessAppReconciler) reconcileCreate(ctx context.Context, app *platformerv1.ServerlessApp) (ctrl.Result, error) {
	...
	roleName := fmt.Sprintf("platformer-%s-%s", app.Namespace, app.Name)
	fnName := fmt.Sprintf("platformer-%s-%s", app.Namespace, app.Name)
	...
	if app.Spec.Database != nil {
		...
		if _, err := r.Provider.CreateDatabase(ctx, provider.DatabaseSpec{
			Name:   fnName,
			Tables: tables,
		}); err != nil {
...
func (r *ServerlessAppReconciler) reconcileDelete(ctx context.Context, app *platformerv1.ServerlessApp) (ctrl.Result, error) {
	...
	_ = r.Provider.DeleteExecutionRole(ctx, roleName)
